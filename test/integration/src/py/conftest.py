...
def with_snapshot(snapshot_name):
    make_py_module.cleanup_and_reset_state()
    ctx = IntegrationTestContext(snapshot_name)
    logging.info("Started processes: {}".format(repr(ctx.processes)))
    threadlocals.ctx = ctx
    os.environ["VAGRANT_ENV_JSON"] = make_py_module.project_dir("test/integration/vagrantenv.json")  # TODO HACK
    print("Before with_snapshot()")
    yield
    print("After with_snapshot()")
    make_py_module.killall(ctx.processes)  # TODO Ensure this is called even in the case of exception
    logging.info("Terminated processes: {}".format(repr(ctx.processes)))
    del threadlocals.ctx

@pytest.fixture
def ctx(snapshot_name):
    print("Before test_context()")
    yield threadlocals.ctx
    print("After test_context()")
